...
<script>
    ...
    function DocumentStart() {
        ...
        LayoutLeft = LayoutMain.cells("a").attachLayout({
                ...
                cells: [
                    ...
                    {id: "c", header: true, text:"Size Prepress", width: 350},
        ...
        LayoutLeft.cells("a").setWidth(LayoutLeft.cells("d").getWidth() - 640);
        ...
        ToolbarMain.addButton("PrintDueDate", null, "In (Có Duedate)", "save.gif");
        ToolbarMain.addButton("InBu", null, "Tạo Lệnh In Bù", "save.gif");
        ...
        ToolbarMain.attachEvent("onClick", function(id) {
            if(id == "FOD") {
                ...
                DataPEForm.setItemValue("RemarkJJ", "In Bù");
                if(DataPEForm.getItemValue("SOLine",true).indexOf("-B") === -1) DataPEForm.setItemValue("SOLine", DataPEForm.getItemValue("SOLine",true) + "-B");
                DataPEForm.setItemValue("ReceivingDate", "<?php echo date("Y-m-d"); ?>");
                DataPEForm.setItemValue("RequestDate", "<?php echo date("Y-m-d", strtotime("+2 days")); ?>");
                DataPEForm.setItemValue("Dueday", "<?php echo date("Y-m-d", strtotime("+2 days")); ?>");
                DataPEForm.setItemValue("PromiseDate", "<?php echo date("Y-m-d", strtotime("+2 days")); ?>");
            } else if(id == "FODNULL") {
            ...
            } else if(id == "PrintDueDate") {
                var JobJacket = DataPEForm.getItemValue("JobJacket", true);
                if(JobJacket != "") window.open("PrintPageNew.php?JJ=" + JobJacket, '_blank'); 
                else dhtmlx.alert("Chưa làm lệnh không được in :P");
            } else if(id == "Save") {
                if(DataPEForm.getItemValue("NumSize", true).toString().indexOf("N") !== -1) {
                    dhtmlx.alert("Vui Lòng Nhập Số Size");
                ...
                ToolbarMain.disableItem("SavePrint");
                var JobJacket = SaveOrder();
                console.log("JobJacket: " + JobJacket);
        ...
        ToolbarMainData.addButton("deleteTeeth", null, "Delete Teeth", "save.gif");
        ...
        ToolbarMainData.attachEvent("onClick", function(id){
            if(id == "InputSize") {
                ...
                GridSize.clearAll();
                UnlockItem();
            } else if(id == "InsertItem") {
                InsertNewItem(DataPEForm.getItemValue("ItemCode", true));
            } else if(id == "deleteTeeth") {
                deleteTeeth();
            }
        });
        InitGrid();
    ...
    function InitGrid(){
        ...
        GridSO.setHeader("SO Line, Item, Customer, Qty, Request, Size, RBO, CRD, PD,JOB,Remark");		
        ...
        GridSOSelect.setHeader("SO Line, Item, Customer, Qty, Request, Size, RBO, CRD, PD, Remark,O,L");
        ...
        GridPrepress.setHeader("Length, Width, Teeth, Last Use, Del");
        GridPrepress.setInitWidths("60,60,60,*,35")
        GridPrepress.setColAlign("center,center,center,center,center");
        GridPrepress.setColTypes("ro,ro,ro,ro,ch");
        GridPrepress.setColSorting("str,str,str,str,na");	
    ...
    function deleteTeeth(){
		var checkIDs = [];
		GridPrepress.forEachRow(function(id){
			if(GridPrepress.cells(id,4).getValue()==1){ checkIDs.push(id.toString()); }
		});
		if(!checkIDs.length>0){
			alert("Vui lòng chọn dòng để XÓA ");
			return false;
		}else{
			var confirm_delete = confirm("Bạn có chắc chắn muốn XÓA Length đã chọn? ");
			if(confirm_delete){
                var DataSend = {"EVENT":"DELETELENGTH"};
                DataSend.ITEMCODE = DataPEForm.getItemValue("ItemCode", true);
                DataSend.DELETELENGTHLIST = JSON.stringify(checkIDs);
                console.log('DataSend: ' + JSON.stringify(DataSend));
                var Result = AjaxAsync("Event.php", DataSend, "POST", "JSON" );
                if (Result == true ) { alert("Đã xóa Length thành công"); } 
                else { alert("Có lỗi khi xóa Length: " + Result ); }
                location.reload();
			}
		}	
	}
    ...
    function ComputeIndex() { // Tính Scrap
        ...
        let MaterialCode = DataPEForm.getItemValue("MaterialCode", true);
        ...
        var FScrap = 0;
        // @tandoan: 20200909 - Thay đổi mới nhất theo mail SCRAP FORMULA CHANGE (giang.phan)
        if(Qty < 101) FScrap = 0.21; // 21%
        else if(Qty > 100 && Qty < 201) FScrap = 0.12; // 12%
        else if(Qty > 200 && Qty < 301) FScrap = 0.08; // 8%
        else if(Qty > 300 && Qty < 501) FScrap = 0.06; // 6%
        else if(Qty > 500 && Qty < 801) FScrap = 0.04; // 4%
        else if(Qty > 800 && Qty < 1001) FScrap = 0.03; // 3%
        else if(Qty > 1000 && Qty < 2001) FScrap = 0.03; // 3%
        ...
        var A = 0;
        var B = 15; // Thay đổi B=30 thành 15
        var C = 2;
        var F = 0;
        var H = 0;
        ...
        // @TanDoan - 20210423: mail: SCRAP FORMULA CHANGE 
        if (MaterialCode == 'THWE004203' ) {
            F = 1.04;
        }
        ...
        // @Tandoan 20201130: Đơn hàng FOD xử lý lại theo mail: Buffer time and material for FOD
        // - Đơn FOD, code mực màu đen hoặc trắng: cộng thêm 100 yard thay vì 200 yard.
        // - Đơn FOD, code mực màu, cộng thêm 300 yard thay vì 500 yard. 
        if(FOD != undefined) {
            if(DataPEForm.getItemValue("InkCode", true).toUpperCase().trim() == "BLACK" || DataPEForm.getItemValue("InkCode", true).toUpperCase().trim() == "WHITE") QtyNeed = QtyNeed + 100;
            else QtyNeed = QtyNeed + 300;
            
            console.log('QtyNeed FOD: '+QtyNeed);
        }
    ...
    function ReloadSize() {
        ...
        for (var i=0; i< GridSOSelect.getRowsNum(); i++){
            ...
            if(PromiseDate > ConvertDate(GridSOSelect.cells2(i,8).getValue()) || TurnDate) {
                if(GridSOSelect.cells2(i,8).getValue() != "") {
                    PromiseDate = ConvertDate(GridSOSelect.cells2(i,8).getValue());
                } 
                else PromiseDate = '';
            }
            TurnDate = false;
        ...
        if(InBu) SOL += "-B";
        DataPEForm.setItemValue("SOLine", SOL);
        ...
        if(PromiseDate != '' && PromiseDate != "Invalid Date") {
            DataPEForm.setItemValue("PromiseDate", FormatDate(PromiseDate));
        } 
    ...
    function InsertNewItem (itemcode) {
        ...
        ItemForm.attachEvent("onButtonClick", function(name){
            ...
            {
                window.open("http://147.121.59.254/intranet/artWork/index.php?KEYWORD=" + ItemCode + "*VN", '_blank'); 
            }
    ...
    function SaveOrder(){
        ...
        if (InBu ) {
            ...
            var JobJacket = DataPEForm.getItemValue("JobJacket", true);
            for(var idC=2; idC<GridSize.getColumnsNum(); idC++ ) {
                var so_line = GridSize.getColLabel(idC);
                GridSize.forEachRow(function(id){    
                    var size = this.cells(id,1).getValue();
                    var qty = this.cells(id,idC).getValue();
                    size_item = {
                        production_line: 'pfl',
                        no_number: JobJacket,
                        so_line: so_line,
                        ...
                        GridSO.forEachRow(function(id){
                            if(id == SOLINE) {
                                if(GridSO.cells(id,9).getValue().indexOf("-") !== -1) {
                                    ...
                                    if(confirm("Đơn đã làm lệnh! bạn có muốn sửa đơn")) {
                                        GridSOSelect.clearAll();
                                        GridSOSelect.loadXML("Data.php?EVENT=LOADSOFROMJOB&JOBJACKET=" + GridSO.cells(id,9).getValue(),function(){
                                            GridPrepress.clearAll();
                                            GridPrepress.loadXML("Data.php?EVENT=LOADSIZE&ITEM=" + GridSO.cells(id,1).getValue(),function(){
                                                if(GridPrepress.getRowsNum() != 0) {
                                                ...
                                                FillData(GridSO.cells(id,1).getValue());
                                                ReloadSize();
                                                var DataRaw = AjaxAsync("Data.php", {"EVENT":"LOADJOBJACKET","JOBJACKET":GridSO.cells(id,9).getValue()},"GET","JSON");
                                                if(DataRaw.length == 1) {
    ...
    var DataForm = [
        {type: "settings", position: "label-left", labelWidth: 90, inputWidth: 130},
            ...
            {_idd: "1108", type: "block", width: "auto", blockOffset: "label-right", list: [
                ...
                {_idd: "1164", type: "newcolumn"},
                {_idd: "1165", type: "radio", label: "Cắt Nhiệt", value: "FUSE CUT", name: "CutMethod"},
            ]},
    ...
    var DataFormInsert = [
        ...
        {type: "fieldset", offsetLeft:20, offsetTop: 20, label: "Main Information", width: "auto", blockOffset: 0, list: [
            ...
            {_idd: "1108", type: "block", width: "auto", blockOffset: "label-right", offsetTop: 10, list: [
                ...
                {_idd: "1150", type: "newcolumn"},
                {_idd: "1159", type: "radio", label: "Thẳng Nguội", value: "DIE CUT", name: "CutMethod"},
                {_idd: "1160", type: "radio", label: "Không Cắt", value: "NONE", name: "CutMethod"},
                {_idd: "1165", type: "radio", label: "Cắt Nhiệt", value: "FUSE CUT", name: "CutMethod"},
            ]},
